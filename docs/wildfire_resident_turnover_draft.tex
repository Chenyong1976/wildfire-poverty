...
2012 Wildfire Hazard Potential raster.
Counter to conventional displacement narratives,
in-migration rates rise by 1.17 percentage points, driven by the increase
in the number of in-migrants with a constant base population.
This indicates residential turnover instead of net population inflow.
The declining renter-unit counts and rising owner-occupancy shares are
consistent with a sorting mechanism in which lower-income renters exit
and owner-occupiers arrive. Poverty, income, and employment show
no statistically significant effects, consistent with compositional replacement
of the resident population rather than incumbent impoverishment.
...
\begin{enumerate}
  \item Keep the use of 2012 WHP as a robustness check. Use 2014 WHP as the baseline.
  \item In the matching, mean WHP , quintile of WHP and distance to the nearest high-hazard pixel are used. In the covariate balance table, only mean WHP is reported. All three should be reported. The details of how these three variables are used and why should be explained in the matching section of the paper.
  \item The normalized difference in WHP is 0.43, which is too high. suggest methods to improve the matching. how about using calibers.
  \item In the matching, socioeconomic covariates should include RUCC
  \item Table 1 of summary statistics should include all the variables included in the matching.
  \item
\end{enumerate}
...
since 1984. This first-fire design eliminates the confound factors
introduced by accumulated fire history and the fire induced adaptation.
...
and the Wald's pre-trend test.
The finding survives Rambachan-Roth sensitivity analysis at breakdown value
$M = 1$, when the undetected pre-trend is larger in
...
to rise by 1.17 percentage points in census tracts experiencing 
their first large fire in recent history. The effect is persistent across 
three post-treatment periods.
Total population is unchanged, confirming that
the rate increase reflects compositional change, roughly equal elevated
in- and out-migration, rather than net population growth.
...
a large fire. First, a reconstruction-labor channel: contractors, construction
workers, and insurance professionals move into fire-damaged areas to
support rebuilding. This predicts a front-loaded spike at the first post-treatment period that
subsequently decays as reconstruction concludes.
Second, a return-migration channel: residents who evacuated
...
observable post-period. Third, a compositional-sorting channel: fires accelerate
...
amenity valuations. This predicts a persistent effect that grows over time as the composition
...
We extend both papers by showing
that simultaneous in-migration and out-migration leading to a compositional change
rather than a net population change.
...
channel. \citet{borgschulte2024air} document that wildfire smoke reduces quarterly
earnings by 0.1 percent per smoke day. We complement these papers with the first nationally representative,
...
\subsection{Fire Treatment and Control Pool}
...
A critical additional restriction is the first-fire design.
...
raster at native 270-meter resolution\footnote{WHP is a modeled output---a statistical combination of vegetation type,
...
pre-treatment period.}: mean WFP percentile,
